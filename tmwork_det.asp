...
<head>
    ...
    <title>iQOR | Work Location Details</title>
    ...
    <%
    sWork_ID = UCase(request("txtWork_ID"))
    ...
    if sModeSub <> "" Then
        ...
        if sModeSub = "up" Then
            ...
            sSQL = "UPDATE TMWORK SET "             
            sSQL = sSQL & "PART = '" & pRTIN(sPart) & "'"
            sSQL = sSQL & " WHERE WORK_ID = '" & sID & "'"
            conn.execute sSQL
        
            call confirmBox("Update Successful!", sMainURL&sAddURL&"&txtWork_ID=" & sID & "")

        elseif sModeSub = "save" Then
            
            sSQL = "insert into TMWORK (WORK_ID,PART,USER_ID,DATETIME) "
		    sSQL = sSQL & "values ("
		    sSQL = sSQL & "'" & pRTIN(sID) & "',"		
		    ...
		    call confirmBox("Save Successful!", sMainURL&sAddURL&"&txtWork_ID=" & sID & "")    
...
<body class="hold-transition skin-blue sidebar-mini">
            ...
            <section class="content">
                <div class="row">
                    <div class="col-md-12">
                        <!-- Horizontal Form -->
                        <!-- form start -->
                        <form id="form1" class="form-horizontal" action="tmwork_det.asp" method="post">
                            ...
                            <div class="box box-info">
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-7">
                                            <%if sWork_ID <> "" then %>
                                                <span class="mod-form-control"><% response.write sWork_ID%> </span>
                                                <input type="hidden" id="txtID" name="txtID" value='<%=sWork_ID%>' />
                                            <%else%>  
                                ...
                                <div class="box-footer">
                                    <%if sWork_ID <> "" then %>
                                        ...
                                        onclick ="fDel('<%=sWork_ID%>','mycontent-del','#mymodal-del')">Delete</a>
                                        <button type="submit" id="btnUp" name="sub" value="up" class="btn btn-info pull-right" style="width: 90px">Update</button>
                                    <%else %>
                                        <button type="button" class="btn btn-primary pull-right" style="width: 90px" onclick="check('WORK');">Save</button>
    ...
    function showDelmodal(str,pContent){
        var xhttp;
  	    ...
  	    xhttp.open("GET", "tmwork_del.asp?txtstring="+str, true);
  	    xhttp.send();
