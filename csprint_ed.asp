...
private sub pageHeader()
	...
	response.write "<table cellSpacing=0 cellpadding=0 width=900 >"
		response.write "<tr>"
			response.write "<td width=100 align=left> Report : Employee Details Report</td>"
	...
	response.write "<tr>"
		...
		response.write "<td align=left width=150>Full Name</td>"
		response.write "<td align=left width=60>Date</td>"
		if sDisType = "N" then 
		...
		elseif sDisType = "B" then 
			...
			response.write "<td align=right width=48>Total </td>"
		else
			response.write "<td align=right width=50>Extra (RM)</td>"
			' response.write "<td align=right width=48>Total </td>"
		end if
	...
	response.write "<table cellSpacing=0 cellpadding=0 width=900 class='fontrptdetail' style='table-layout: fixed;'>"
...
%>
		...
		if not rstCSTrns.eof then
			...
			do while not rstCSTrns.eof
					...
					dTotCoupon = dCoupon + dECoupon	
					 ...
					 if rstCSTrns("EMP_CODE") <> sPrevEmpCode then
						...
						if sDisType = "B" then 
							response.write "<td colspan=6><hr width=30% align=right size=1 noshade style='margin-top: 0px;margin-bottom: 0px'></td>"
						else
							response.write "<td colspan=6><hr width=10% align=right size=1 noshade style='margin-top: 0px;margin-bottom: 0px'></td>"
						end if
						response.write "</tr>"
					 	response.write "<tr valign=top>"
					 	response.write "<td align=left width=120></td>"
						response.write "<td align=left width=300></td>"
						response.write "<td align=left width=180></td>"
						if sDisType = "N" then 
							response.write "<td align=right width=50>" & pFormatDash(dSubTotCoupon,2) & "</td>"
						elseif sDisType = "E" then
							response.write "<td align=right width=100>" & pFormatDash(dSubTotECoupon,2) & "</td>"
							' response.write "<td align=right width=50>" & pFormatDash(dSubTotal,2) & "</td>"
						elseif sDisType = "B" then
							response.write "<td align=right width=50>" & pFormatDash(dSubTotCoupon,2) & "</td>"
							response.write "<td align=right width=100>" & pFormatDash(dSubTotECoupon,2) & "</td>"
							response.write "<td align=right width=100>" & pFormatDash(dSubTotal,2) & "</td>"
						end if
						response.write "</tr>"
						response.write "<tr valign=top>"
						if sDisType = "B" then 
					...
					if bPrint = true then
						if sDisType = "B" then
							response.write "<td align=left width=120>" & sEmpCode1 & "</td>"
							response.write "<td align=left width=300>" & rstCSTrns("NAME") & "</td>"
						else
							response.write "<td align=left width=165>" & sEmpCode1 & "</td>"
							response.write "<td align=left width=425>" & rstCSTrns("NAME") & "</td>"
						end if
						bPrint = false
					else
						if sDisType = "B" then
							...
							response.write "<td align=left width=425></td>"
						end if
					end if
					
					if sDisType = "B" then
						response.write "<td align=left width=180>" & fdatelong(rstCSTrns("DT_TRNS")) & "</td>"
					elseif sDisType = "N" then
						response.write "<td align=left width=260>" & fdatelong(rstCSTrns("DT_TRNS")) & "</td>"
					elseif sDisType = "E" then
						response.write "<td align=left width=200>" & fdatelong(rstCSTrns("DT_TRNS")) & "</td>"
					end if
					
					if sDisType = "N" then
						response.write "<td align=right width=50>" & pFormatDash(dCoupon,2) & "</td>"
					elseif sDisType = "E" then
						response.write "<td align=right width=100>" & pFormatDash(dECoupon,2) & "</td>"
						' response.write "<td align=right width=50>" & pFormatDash(dTotCoupon,2) & "</td>"
					elseif sDisType = "B" then
						response.write "<td align=right width=50>" & pFormatDash(dCoupon,2) & "</td>"
						response.write "<td align=right width=100>" & pFormatDash(dECoupon,2) & "</td>"
						response.write "<td align=right width=100>" & pFormatDash(dTotCoupon,2) & "</td>"
					end if
					response.write "</tr>"
						
					dSubTotCoupon = dSubTotCoupon + dCoupon
					dSubTotECoupon = dSubTotECoupon + dECoupon
					dSubTotal = dSubTotal + dTotCoupon	
			...
			loop	
						...
						if sDisType = "N" then
							response.write "<td align=right width=50>" & pFormatDash(dSubTotCoupon,2) & "</td>"
						elseif sDisType = "E" then
							response.write "<td align=right width=50>" & pFormatDash(dSubTotECoupon,2) & "</td>"
							' response.write "<td align=right width=50>" & pFormatDash(dSubTotal,2) & "</td>"
						elseif sDisType = "B" then
							response.write "<td align=right width=50>" & pFormatDash(dSubTotCoupon,2) & "</td>"
							response.write "<td align=right width=50>" & pFormatDash(dSubTotECoupon,2) & "</td>"
	...
	<table cellSpacing=0 cellpadding=0 width=900 class="fontrptdetail">
		<tr>
		...
		if not rstCSTrns.eof then
			...
			response.write "<td align=right width=280>Total : </td>"
			if sDisType = "N" then
				response.write "<td align=right width=50>" & pFormatDash(dCoupon,2) & "</td>"
			elseif sDisType = "E" then
				response.write "<td align=right width=50>" & pFormatDash(dECoupon,2) & "</td>"
				' response.write "<td align=right width=48>" & pFormatDash(dTotCoupon,2) & "</td>"
			elseif sDisType = "B" then
				response.write "<td align=right width=50>" & pFormatDash(dCoupon,2) & "</td>"
				response.write "<td align=right width=50>" & pFormatDash(dECoupon,2) & "</td>"
				response.write "<td align=right width=48>" & pFormatDash(dTotCoupon,2) & "</td>"
