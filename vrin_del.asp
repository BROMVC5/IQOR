...
<%
    ...
    iAutoInc = request("txtstring")
	sIC = request("txtAutoInc")
	
	Set rstVRTrns = server.CreateObject("ADODB.RecordSet")    
		sSQL = "select * from vrtrns where AUTOINC ='" & iAutoInc & "'"
    ...
    if reqForm("btnSubmit") <> "" then
        ...
        sSQL = "delete from VRTRNS where AUTOINC='" & iAutoInc & "'"
        conn.execute sSQL     
        
        sMainURL = "vrin.asp?"
        sAddURL = "txtAutoInc=" & iAutoInc & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="vrin_del.asp" method="post">
        <input type="hidden" name="txtAutoInc" value='<%=iAutoInc%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>NRIC : <% response.write sIC %></h3></span>
					<span style="color:blue"><h3>Badge No : <% response.write sBadge_No %></h3></span>
