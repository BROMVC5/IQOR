...
<head>
	...
	Set rstvrOver = server.CreateObject("ADODB.RecordSet")    
    sSQL = "SELECT COUNT(NRIC) AS TOTALOVER ,DT_OT FROM VRTRNS "
    sSQL = sSQL & "WHERE DT_OT IS NULL "	
    rstvrOver.Open sSQL, conn, 3, 3 
    if not rstvrOver.eof then
    	iTotalOver = rstvrOver("TOTALOVER")         
    end if
    pCloseTables(rstvrOver)
    %>
</head>
                ...
                <div class="row">
                    <div class="col-xs-12">
						...
						<div class="small-box bg-purple color-palette">
						    <div class="inner">
						      <h3>Overdue : <% response.write iTotalOver %></h3>
						
						      <p>Total Overdue</p>
						    </div>
						    <div class="icon">
						      <i class="fa ion-thumbsdown"></i>
						    </div>
						    <a href="vrout.asp" class="small-box-footer">More info <i class="fa fa-arrow-circle-right"></i></a>
						  </div>
						</div>
