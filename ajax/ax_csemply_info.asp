...
sql = "select CARDNO, EMP_CODE, NAME, COUPON from csemply "
if sql_1 <> "" then
	sql = sql & sql_1
end if 
...
rstcsemply.cursortype = adOpenStatic
rstcsemply.cursorlocation = adUseClient
rstcsemply.locktype = adLockBatchOptimistic
rstcsemply.pagesize = PageLen		
rstcsemply.Open sql, conn, 3, 3

'**************** Paging/Pagination Calculator ***************'
If not rstcsemply.eof then
 	rstcsemply.absolutepage = iCurPage
 	iPageCount = rstcsemply.PageCount
end if 

PageNo = Request.QueryString("Page")
if PageNo = "" Then PageNo = 1
TotalRecord = rstcsemply.RecordCount
    ...
    <table id="example1" class="table table-bordered table-striped">
        <thead>
            <tr>
                ...
                <th style="width:13%;text-align:right">Subsidy Amount (RM)</th>
        ...
        <tbody>
            <%
                ...
                do while not rstcsemply.eof and i < iRecCount
                                            
                sURL = "&page=" & PageNo & "&txtsearch=" & txtSearch
                i = i + 1                             
                response.write "<tr>"
                response.write "<td>" & i + ((PageNo-1)*PageLen) & "</td>"
                response.write "<td>" & rstcsemply("EMP_CODE") & "</td>"
                response.write "<td>" & rstcsemply("NAME") & "</td>"
                response.write "<td>" & rstcsemply("CARDNO") & "</td>"
                response.write "<td style=""text-align:right"">" & pFormatDec(rstcsemply("COUPON"),2) & "</td>"
                response.write "<td style=""text-align:center""><a href='csemply_det.asp?page="& PageNo & "&txtsearch=" & txtSearch & "&txtEMP_CODE="& rstcsemply("EMP_CODE") & "'><img src=""dist/img/edit-2-24.png"" /></a></td>"
                response.write "</tr>"
                rstcsemply.movenext
            loop
