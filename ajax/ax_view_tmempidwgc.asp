...
if txtSearch <> "" then
	ScStr = txtSearch
 	ScStr = replace(ScStr,"'","''")
  	if sWorkGrp_ID ="" or sSup_Code = "" then
...
if sWorkGrp_ID <> "" and sSup_Code <> "" then
    sql = sql & " where EMP_CODE in (SELECT EMP_CODE from TMWORKGRP where WORKGRP_ID = '" & sWorkGrp_ID & "')"
    sql = sql & " and SUP_CODE = '" & sSUP_CODE & "'" 
elseif sWorkGrp_ID = "" and sSup_Code <> "" then
    sql = sql & " where SUP_CODE = '" & sSUP_CODE & "'"
elseif sWorkGrp_ID <> "" and sSup_Code = "" then
    sql = sql & " where EMP_CODE in (SELECT EMP_CODE from TMWORKGRP where WORKGRP_ID = '" & sWorkGrp_ID & "')"
end if 
    ...
    <table id="example1" class="table table-bordered">
        ...
        <tbody>
            ...
            do while not rstUser.eof and i < iRecCount
                ...
                i = i + 1
                response.write "<tr onmouseover=this.bgColor='#FFF59D' onmouseout=this.bgColor='white' onclick=""getValue('" & rstUser("EMP_CODE") & "','txtID')"">"
