...
sApprove = trim(request("txtApprove"))

if sApprove = "undefined" then
    sApprove = ""
end if 
...
if txtSearch <> "" then
	ScStr = txtSearch
 	ScStr = replace(ScStr,"'","''")
  	sql_1 = "where (cpresv.ticket_no like '%" & ScStr & "%') "
  	sql_1 = sql_1 & " or (cpresv.r_name like '%" & ScStr & "%') "
	sql_1 = sql_1 & " or (cpresv.car_no like '%" & ScStr & "%') "
end if

sql = "select cpresv.ticket_no, cpresv.r_name, cpresv.car_no, cpresv.d_in, cpresv.d_out, cpresv.t_in, cpresv.t_out, tmemply.name, cpresv.approve from cpresv "
sql = sql & "left join tmemply on cpresv.emp_code = tmemply.emp_code "
if sApprove <> "" then
    sql = sql & " where APPROVE = '" & sApprove & "'"
else
    sql = sql & " where 1=1 "
end if
        ...
        <tbody>
            <%
                ...
                If rstRCReserve("APPROVE") = "Y" then
                    response.write "<td><b style = 'color:green'>Approved</b></td>"
                ElseIf rstRCReserve("APPROVE") = "N" then
                    response.write "<td><b style = 'color:red'>Rejected</b></td>"
                Else
                    response.write "<td><b style = 'color:blue'>Pending</b></td>"
                End If
                response.write "<td style=""width:2%;text-align:center""><a href='cpreserve_det.asp?page="& PageNo & "&txtsearch=" & txtSearch & "&txtTicket_No="& rstRCReserve("TICKET_NO") & "'><img src=""dist/img/edit-2-24.png"" /></a></td>"
                response.write "</tr>"
