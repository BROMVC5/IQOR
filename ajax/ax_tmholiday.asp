...
sql = sql & "order by dt_hol desc "

set rstTMHol = server.createobject("adodb.recordset")
rstTMHol.cursortype = adOpenStatic
rstTMHol.cursorlocation = adUseClient
rstTMHol.locktype = adLockBatchOptimistic
rstTMHol.pagesize = PageLen		
rstTMHol.Open sql, conn, 3, 3

'**************** Paging/Pagination Calculator ***************'
If not rstTMHol.eof then
 	rstTMHol.absolutepage = iCurPage
 	iPageCount = rstTMHol.PageCount
end if 

PageNo = Request.QueryString("Page")
if PageNo = "" Then PageNo = 1
TotalRecord = rstTMHol.RecordCount
    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
            <%
                ...
                do while not rstTMHol.eof and i < PageLen
                                            
                sURL = "&page=" & PageNo & "&txtsearch=" & txtSearch
                i = i + 1                             
                response.write "<tr>"
                response.write "<td>" & i + ((PageNo-1)*PageLen) & "</td>"
                response.write "<td>" & rstTMHol("dt_hol") & "</td>"
                response.write "<td>" & rstTMHol("part") & "</td>"
                response.write "<td style=""width:2%;text-align:center""><a href='tmholiday_det.asp?page="& PageNo & "&txtsearch=" & txtSearch & "&txtDT_HOL="& rstTMHol("dt_hol") & "'><img src=""dist/img/edit-2-24.png"" /></a></td>"
                response.write "</tr>"
                rstTMHol.movenext
            loop
