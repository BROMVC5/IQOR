...
sql = "select cpresv.ticket_no, cpresv.r_name, cpresv.car_no, cpresv.d_in, cpresv.d_out, cpresv.t_in, cpresv.t_out, cpresv.approve, tmemply.name from cpresv "
sql = sql & "left join tmemply on cpresv.emp_code = tmemply.emp_code "
sql = sql & " where cpresv.approve = 'N' "
if sql_1 <> "" then
	sql = sql & sql_1
end if 
    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
            <%
                ...
                response.write "<tr>"
                response.write "<td>" & rstRCReserve("ticket_no") & "</td>"
				...
				if rstRCReserve("approve") = "Y" then
               		response.write "<td style=""text-align:center""><b style='color:green'>Approved</b></td>" 
                elseif rstRCReserve("approve") = "N" then
                	response.write "<td style=""text-align:center""><b style='color:red'>Rejected</b></td>" 
				else
					response.write "<td style=""text-align:center""><b style='color:grey'>Pending</b></td>"
				end if
                response.write "</tr>"
            ...
            <div class="dataTables_paginate">
                <ul class="pagination">
                    <%IF Cint(PageNo) > 1 then %>
                        <li class="paginate_button"><a href="javascript:showRej('page=1');" class="button_a" ><< First</a></li>
                        <li class="paginate_button"><a href="javascript:showRej('page=<%=PageNo-1%>');" class="button_a" >< Back</a></li>
                    <%End IF%>
                    ...
                    <% if (intID >= (Cint(PageNo)-3)) and (intID <= (Cint(PageNo)+3)) Then%>
                        ...
                        <%Else%>
                            <li class="paginate_button"><a href="javascript:showRej('page=<%=intID%>');" class="button_a" ><%=intID%></a></li>
                        <%End IF%>
                    <%End IF%>
                    <%Next%>

                    <%IF Cint(PageNo) < TotalPage Then %>
                        <li class="paginate_button"><a href="javascript:showRej('page=<%=PageNo+1%>');" class="button_a" >Next ></a></li>
                        <li class="paginate_button"><a href="javascript:showRej('page=<%=TotalPage%>');" class="button_a" >Last >></a></li>
                    <%End IF%>
                </ul>
            </div>
        </div>
