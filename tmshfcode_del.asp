...
<%
    ...
    sSHF_CODE = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtSHF_CODE")
        
        sSQL = "delete from TMSHFCODE where SHF_CODE='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmshfcode.asp?"
        sAddURL = "txtSHF_CODE=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmshfcode_del.asp" method="post">
        <input type="hidden" name="txtSHF_CODE" value='<%=sSHF_CODE%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Shift Code : <% response.write sSHF_CODE %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Shift Code?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
