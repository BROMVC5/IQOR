...
<head>
    ...
    <title>iQOR | Religion Details</title>
    ...
    <%
        ...
        if sModeSub <> "" Then
            ...
            if sModeSub = "up" Then
                ...
                sSQL = "UPDATE TMRELIG SET "             
                sSQL = sSQL & "PART = '" & pRTIN(sPart) & "'"
                sSQL = sSQL & " WHERE RELIG = '" & sID & "'"
                conn.execute sSQL
        
                call confirmBox("Update Successful!", sMainURL&sAddURL&"&txtRELIG=" & sID & "")

            elseif sModeSub = "save" Then
            
                sSQL = "insert into TMRELIG (RELIG,PART,USER_ID,DATETIME) "
		        sSQL = sSQL & "values ("
		        sSQL = sSQL & "'" & pRTIN(sID) & "',"		
		        ...
		        call confirmBox("Save Successful!", sMainURL&sAddURL&"&txtRELIG=" & sID & "")    
        ...
        rstTMRelig.Open sSQL, conn, 3, 3
            if not rstTMRelig.eof then
                sPart = rstTMRelig("PART")
            end if
        pCloseTables(rstTMRelig)
...
<body class="hold-transition skin-blue sidebar-mini">
            ...
            <section class="content">
                <div class="row">
                    <div class="col-md-12">
                        <!-- Horizontal Form -->
                        <!-- form start -->
                        <form id="form1" class="form-horizontal" action="tmrelig_det.asp" method="post">
                            ...
                            <div class="box box-info">
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-7">
                                            <%if sRELIG <> "" then %>
                                                <span class="mod-form-control"><% response.write sRELIG%> </span>
                                                <input type="hidden" id="txtID" name="txtID" value='<%=sRELIG%>' />
                                            <%else%>  
                                ...
                                <div class="box-footer">
                                    <%if sRELIG <> "" then %>
                                        ...
                                        onclick ="fDel('<%=sRELIG%>','mycontent-del','#mymodal-del')">Delete</a>
                                        <button type="submit" id="btnUp" name="sub" value="up" class="btn btn-info pull-right" style="width: 90px">Update</button>
                                    <%else %>
                                        <button type="button" class="btn btn-primary pull-right" style="width: 90px" onclick="check('RELIG');">Save</button>
    ...
    function showDelmodal(str,pContent){
        var xhttp;
  	    ...
  	    xhttp.open("GET", "tmRELIG_del.asp?txtstring="+str, true);
  	    xhttp.send();
