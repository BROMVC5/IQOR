...
<%
    ...
    sOTCode = request("txtstring")
   
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtOTCode")
      
        sSQL = "delete from TMOTCODE where OTCODE = '" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmotcode.asp?"
        sAddURL = "txtOTCode=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmotcode_del.asp" method="post">
        <input type="hidden" name="Page" value='<%=iPage%>' />
        <input type="hidden" name="txtOTCode" value='<%=sOTCode%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                     <span style="color: blue"><h3>DELETE OT CODE : <% response.write sOTCode %></h3></span>
