...
<%
    ...
    sHol_ID = request("txtstring")
   
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtHOL_ID")
      
        sSQL = "delete from TMHOL1 where HOL_ID = '" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmholcal.asp?"
        sAddURL = "txtHOL_ID=" & dt_hol & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmholcal_del.asp" method="post">
        <input type="hidden" name="Page" value='<%=iPage%>' />
        <input type="hidden" name="txtHOL_ID" value='<%=sHol_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                     <span style="color: blue"><h3>DELETE HOLIDAY GROUP CODE : <% response.write sHOL_ID %></h3></span>
                     <span style="color: red"><h4>Are you sure to delete this group?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
