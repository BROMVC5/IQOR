...
<%
    ...
    sDESIGN_ID = request("txtstring")

    if sDESIGN_ID <> "" then
        sID = sDESIGN_ID
    else
        sID = reqformU("txtDESIGN_ID")
    end if
  
    if reqForm("btnSubmit") <> "" then
        
        sSQL = "delete from TMDESIGN where DESIGN_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "TMDESIGN.asp?"
        sAddURL = "txtDesign_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
    ...
    sSQL = "select EMP_CODE,NAME, WORK_ID from TMEMPLY" 
    rstTMEMPLY.Open sSQL, conn, 3, 3
        do while not rstTMEMPLY.eof
            if sID = rstTMEMPLY("WORK_ID") then
...
<body>
    <form class="form-horizontal" action="tmdesign_del.asp" method="post">
        <input type="hidden" name="txtDESIGN_ID" value='<%=sDESIGN_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <%if bDel = "Y" then %>
                        <span style="color:blue"><h3>Designation ID : <% response.write sID %></h3></span>
                        <span style="color:red"><h4>Are you sure to delete this designation?</h4></span>
                    <%else%>
                        <span style="color:blue"><h3><% response.write sID %> is used by <%response.write sWho & "-" & sName %></h3></span>
                        <span style="color:red"><h4>Cannot delete this desingation</h4></span>
                    <%end if%>
                </div>
            </div>
        </div>
