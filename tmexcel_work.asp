...
Set objOpenFile = fso.OpenTextFile(sFilePath,8,True)
	...
	if not rstTMWORK.eof then
	    ...
	    sStr = fCol("No") & sep & fCol("Work Location") & sep & fCol("Description") & sep 
        objOpenFile.WriteLine sStr
		
		do while not rstTMWORK.eof
            sRecord = sRecord + 1
		    sStr = sRecord & sep & rstTMWORK("WORK_ID") & sep & rstTMWORK("PART") & sep 
            objOpenFile.WriteLine sStr
