...
<%  
    ...
    sAllCode = request("txtstring")
   
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtAllCode")
      
        sSQL = "delete from TMALLOW where ALLCODE = '" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmallow.asp?"
        sAddURL = "txtAllCode=" & sID 
...
<body>
    <form class="form-horizontal" action="tmallow_del.asp" method="post">
        <input type="hidden" name="txtAllCode" value='<%=sAllCode%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                     <span style="color: blue"><h3>DELETE ALLOWANCE CODE : <% response.write sAllCode %></h3></span>
                     <span style="color: red"><h4>Are you sure to delete this code?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
