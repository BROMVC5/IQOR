...
<%
    ...
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtAreaCode")
        
        sSQL = "delete from tsarea where AREACODE='" & sID & "'"
        conn.execute sSQL   
        
        sMainURL = "tsarea.asp?"
        sAddURL = "txtAreaCode=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tsarea_del.asp" method="post">
        <input type="hidden" name="txtAreaCode" value='<%=sAreaCode%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>AREA CODE : <% response.write sAreaCode %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Area Code?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
