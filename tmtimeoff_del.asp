...
<head>
    ...
    sTOff_ID = request("txtstring")
    
    if sTOff_ID <> "" then
        sID = sTOff_ID
    else
        sID = reqformU("txtTOff_ID")
    end if
    
    if reqForm("btnSubmit") <> "" then
        
        sSQL = "delete from TMTIMEOFF where TOFF_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "TMTIMEOFF.asp?"
        sAddURL = "txtTOff_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="TMTIMEOFF_del.asp" method="post">
        <input type="hidden" name="txtTOff_ID" value='<%=sTOff_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color: blue">
                        <h3>Time Off Code : <% response.write sID %></h3>
                    </span>
                    <span style="color: red">
                        <h4>Are you sure to delete this Code?</h4>
                    </span>
                    <!--<span style="color: blue">
                        <h3><% response.write sID %> is used by <%response.write sWho & "-" & sName %></h3>
                    </span>
                    <span style="color: red">
                        <h4>Cannot delete this work location</h4>
                    </span>-->
                </div>
            </div>
        </div>
