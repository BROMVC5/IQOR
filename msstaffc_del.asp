...
<%
	...
	Set rstVRTrns = server.CreateObject("ADODB.RecordSet")    
		...
		if not rstVRTrns.eof then
			sEmp_ID = rstVRTrns("EMP_CODE")
			sEmp_Name = rstVRTrns("EMP_NAME")			
		end if
  
    if reqForm("btnSubmit") <> "" then
        sTicket_No = reqform("txtTicket_No")
        
        sSQL = "delete from msstaffc where TICKET_NO ='" & sTicket_No & "'"
        conn.execute sSQL     
        
        sMainURL = "msstaffc.asp?"
        sAddURL = "txtTicket_No=" & sTicket_No & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="msstaffc_del.asp" method="post">
        <input type="hidden" name="txtTicket_No" value='<%=sTicket_No%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Ticket No : <% response.write sTicket_No %></h3></span>
					<span style="color:blue"><h4>Employee Name : <% response.write sEmp_Name %></h4></span>
