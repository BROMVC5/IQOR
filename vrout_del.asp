...
<%
    ...
    if reqForm("btnSubmit") <> "" then
        ...
        sMainURL = "vrout.asp?"
        sAddURL = "txtBadge_No=" & sBadge_No & "&txtSearch=" & sSearch & "&Page=" & iPage 

        call confirmBox("Check Out Successful!", sMainURL&sAddURL)
    End If
...
<body>
    <form class="form-horizontal" action="vrout_del.asp" method="post">
        <input type="hidden" name="txtBadge_No" value='<%=sBadge_No%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>BADGE_NO : <% response.write sBadge_No %></h3></span>
                    <span style="color:red"><h4>Are you sure want to Check Out?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
        <div class="box-footer">
            ...
            <input type="submit" name="btnSubmit" value="Check Out" class="btn btn-danger pull-right" style="width: 90px"/>
        </div>
        <!-- /.box-footer --> 
    </form>                  
