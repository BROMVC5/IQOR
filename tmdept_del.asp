...
<%
    ...
    sDEPT_ID = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtDEPT_ID")
        
        sSQL = "delete from TMDEPT where DEPT_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmdept.asp?"
        sAddURL = "txtDEPT_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmdept_del.asp" method="post">
        <input type="hidden" name="txtDEPT_ID" value='<%=sDEPT_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Department ID : <% response.write sDEPT_ID %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Department?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
