...
<%
    ...
    sNATION_ID = request("txtstring")
    
    if sNATION_ID <> "" then
        sID = sNATION_ID
    else
        sID = reqformU("txtNATION_ID")
    end if
    
    if reqForm("btnSubmit") <> "" then
        
        sSQL = "delete from TMNATION where NATION='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "TMNATION.asp?"
        sAddURL = "txtNATION_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
    ...
    sSQL = "select EMP_CODE,NAME, NATION from TMEMPLY" 
    rstTMEMPLY.Open sSQL, conn, 3, 3
        do while not rstTMEMPLY.eof
            if sID = rstTMEMPLY("NATION") then
...
<body>
    <form class="form-horizontal" action="TMNATION_del.asp" method="post">
        <input type="hidden" name="txtNATION_ID" value='<%=sNATION_ID%>' />
        <div class="box-body">
