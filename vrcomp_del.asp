...
<%
    ...
    sComp_ID = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtComp_ID")
        
        sSQL = "delete from VRCOMP where COMPNAME='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "vrcomp.asp?"
        sAddURL = "txtComp_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="vrcomp_del.asp" method="post">
        <input type="hidden" name="txtComp_ID" value='<%=sComp_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Company Name : <% response.write sComp_ID %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Company?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
