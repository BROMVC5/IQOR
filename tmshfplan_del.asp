...
<%
    ...
    sSHFPLAN_ID = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sID = reqform("txtSHFPLAN_ID")
        
        sSQL = "delete from tmshfplan where SHFPLAN_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmshfplan.asp?"
        sAddURL = "txtSHFPLAN_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmshfplan_del.asp" method="post">
        <input type="hidden" name="txtSHFPLAN_ID" value='<%=sSHFPLAN_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Shift Plan : <% response.write sSHFPLAN_ID %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Shift PLan?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
