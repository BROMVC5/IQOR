...
<%
    ...
    sEn_Name = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sEn_Name = reqform("txtEn_Name")
        
        sSQL = "delete from msentype where entitlement='" & sEn_Name & "'"
        conn.execute sSQL     
        
        sMainURL = "msentype.asp?"
        sAddURL = "txtEn_Name=" & sEn_Name & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="msentype_del.asp" method="post">
        <input type="hidden" name="txtEn_Name" value='<%=sEn_Name%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Entitlement : <% response.write sEn_Name %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Entitlement?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
