...
<%
    ...
    swork_ID = request("txtstring")
    
    if swork_ID <> "" then
        sID = swork_ID
    else
        sID = reqformU("txtwork_ID")
    end if
    
    if reqForm("btnSubmit") <> "" then
        
        sSQL = "delete from TMwork where work_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "tmwork.asp?"
        sAddURL = "txtwork_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="tmwork_del.asp" method="post">
        ...
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <%if bDel = "Y" then %>
                        <span style="color:blue"><h3>Work Location : <% response.write sID %></h3></span>
                        <span style="color:red"><h4>Are you sure to delete this work location?</h4></span>
                    <%else%>
                        <span style="color:blue"><h3><% response.write sID %> is used by <%response.write sWho & "-" & sName %></h3></span>
                        <span style="color:red"><h4>Cannot delete this work location</h4></span>
                    <%end if%>
                </div>
            </div>
        </div>
