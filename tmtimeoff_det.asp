...
<head>
    ...
    <title>iQOR | Time Off Code Details</title>
    ...
    <%
    sTOff_ID = UCase(request("txtTOff_ID"))
    ...
    sMainURL = "tmtimeoff.asp?"
	
    sAddURL = "txtSearch=" & sSearch & "&Page=" & iPage & "&txtTOff_ID=" & sID 
                
    if sModeSub <> "" Then
        
        sPart = reqForm("txtPart")
        sPaid = reqForm("selPaid")

        if sModeSub = "up" Then
            
            sSQL = "UPDATE TMTIMEOFF SET "             
            sSQL = sSQL & "PART = '" & pRTIN(sPart) & "',"
            sSQL = sSQL & "PAID = '" & sPaid & "',"
            sSQL = sSQL & "USER_ID = '" & session("USERNAME") & "',"
            sSQL = sSQL & "DATETIME = '" & fdatetime2(Now()) & "'"
            sSQL = sSQL & " WHERE TOff_ID = '" & sID & "'"
            conn.execute sSQL
        ...
        elseif sModeSub = "save" Then
            ...
            sSQL = "insert into TMTIMEOFF (TOff_ID,PART,PAID,CREATE_ID,DT_CREATE,USER_ID,DATETIME) "
		    sSQL = sSQL & "values ("
		    sSQL = sSQL & "'" & pRTIN(sID) & "',"		
...
<body class="hold-transition skin-blue sidebar-mini">
            ...
            <section class="content">
                <div class="row">
                    <div class="col-md-12">
                        <!-- Horizontal Form -->
                        <!-- form start -->
                        <form id="form1" class="form-horizontal" action="TMTIMEOFF_det.asp" method="post">
                            ...
                            <div class="box box-info">
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-7">
                                            <%if sTOff_ID <> "" then %>
                                                <span class="mod-form-control"><% response.write sTOff_ID%> </span>
                                                <input type="hidden" id="txtID" name="txtID" value='<%=sTOff_ID%>' />
                                            <%else%>  
                                    ...
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label">Paid Time Off : </label>
                                        <div class="col-sm-3">
                                            <select name="selPaid" class="form-control">
                                                <option value="" selected="selected">Please Select</option>
                                                <option value="Y" <%if sPaid = "Y" then%>Selected<%end if%>>Yes</option>
                                                <option value="N" <%if sPaid = "N" then%>Selected<%end if%>>No</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                <div class="box-footer">
                                    <%if sTOff_ID <> "" then %>
                                        <a href="javascript:void(0);" class="btn btn-danger pull-left" style="width: 90px"
                                            onclick ="fDel('<%=sTOff_ID%>','mycontent-del','#mymodal-del')">Delete</a>
                                    ...
                                    <%else %>
                                        <button type="button" id="btnCheck" name="btnCheck" class="btn btn-primary pull-right"
                                            style="width: 90px" onclick="check('TOFF');">
                                            Save</button>
    ...
    function showDelmodal(str,pContent){
        var xhttp;
  	    ...
  	    xhttp.open("GET", "tmtimeoff_del.asp?txtstring="+str, true);
  	    xhttp.send();
