...
<head>
    ...
    <title>iQOR | Nationality Details</title>
    ...
    <%
        sNATION = UCase(request("txtNATION"))
        ...
        if sModeSub <> "" Then
            ...
            if sModeSub = "up" Then
                ...
                sSQL = "UPDATE TMNATION SET "             
                sSQL = sSQL & "PART = '" & pRTIN(sPart) & "'"
                sSQL = sSQL & " WHERE NATION = '" & sID & "'"
                conn.execute sSQL
        
                call confirmBox("Update Successful!", sMainURL&sAddURL&"&txtNATION=" & sID & "")

            elseif sModeSub = "save" Then
            
                sSQL = "insert into TMNATION (NATION,PART,USER_ID,DATETIME) "
		        sSQL = sSQL & "values ("
		        sSQL = sSQL & "'" & pRTIN(sID) & "',"		
		        ...
		        call confirmBox("Save Successful!", sMainURL&sAddURL&"&txtNATION=" & sID & "")    
        ...
        rstTMNATION.Open sSQL, conn, 3, 3
            if not rstTMNATION.eof then
                sPart = rstTMNATION("PART")
            end if
        pCloseTables(rstTMNATION)
...
<body class="hold-transition skin-blue sidebar-mini">
            ...
            <section class="content">
                <div class="row">
                    <div class="col-md-12">
                        <!-- Horizontal Form -->
                        <!-- form start -->
                        <form id="form1" class="form-horizontal" action="TMNATION_det.asp" method="post">
                            ...
                            <div class="box box-info">
                                ...
                                <div class="box-body">
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-7">
                                            <%if sNATION <> "" then %>
                                                <span class="mod-form-control"><% response.write sNATION%> </span>
                                                <input type="hidden" id="txtID" name="txtID" value='<%=sNATION%>' />
                                            <%else%>  
                                                <input class="form-control" id="txtID" name="txtID" value="<%=sID%>" maxlength="15" style="text-transform: uppercase" />
                                            <% end if %>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                ...
                                <div class="box-footer">
                                    <%if sNATION <> "" then %>
                                        ...
                                        onclick ="fDel('<%=sNATION%>','mycontent-del','#mymodal-del')">Delete</a>
                                        <button type="submit" id="btnUp" name="sub" value="up" class="btn btn-info pull-right" style="width: 90px">Update</button>
                                    <%else %>
                                        <button type="button" class="btn btn-primary pull-right" style="width: 90px" onclick="check('NATION');">Save</button>
    ...
    function showDelmodal(str,pContent){
        var xhttp;
  	    ...
  	    xhttp.open("GET", "TMNATION_del.asp?txtstring="+str, true);
  	    xhttp.send();
