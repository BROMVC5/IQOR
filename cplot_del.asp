...
<%
    ...
    sLot_ID = request("txtstring")
  
    if reqForm("btnSubmit") <> "" then
        sLID = reqform("txtLot_ID")
        
        sSQL = "delete from RCLOT where LOT_ID='" & sLID & "'"
        conn.execute sSQL     
        
        sMainURL = "rclot.asp?"
        sAddURL = "txtLot_ID=" & sLID & "&txtSearch=" & sSearch & "&Page=" & iPage 
...
<body>
    <form class="form-horizontal" action="rclot_del.asp" method="post">
        <input type="hidden" name="txtLot_ID" value='<%=sLot_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <span style="color:blue"><h3>Lot ID : <% response.write sLot_ID %></h3></span>
                    <span style="color:red"><h4>Are you sure to delete this Lot?</h4></span>
                </div>
            </div>
        </div>
        <!-- /.box-body -->
