...
<%
    ...
    sGrade_ID = request("txtstring")

    if sGrade_ID <> "" then
        sID = sGrade_ID
    else
        sID = reqformU("txtGrade_ID")
    end if
  
    if reqForm("btnSubmit") <> "" then
        
        sSQL = "delete from TMGRADE where GRADE_ID='" & sID & "'"
        conn.execute sSQL     
        
        sMainURL = "TMGRADE.asp?"
        sAddURL = "txtGrade_ID=" & sID & "&txtSearch=" & sSearch & "&Page=" & iPage 
    ...
    sSQL = "select EMP_CODE,NAME, GRADE_ID from TMEMPLY" 
    rstTMEMPLY.Open sSQL, conn, 3, 3
        do while not rstTMEMPLY.eof
            if sID = rstTMEMPLY("GRADE_ID") then
...
<body>
    <form class="form-horizontal" action="TMGRADE_del.asp" method="post">
        <input type="hidden" name="txtGrade_ID" value='<%=sGrade_ID%>' />
        <div class="box-body">
            <div class="form-group">
                ...
                <div class="col-sm-7">
                    <%if bDel = "Y" then %>
                        <span style="color:blue"><h3>Grade Code : <% response.write sGrade_ID %></h3></span>
                        <span style="color:red"><h4>Are you sure to delete this Grade?</h4></span>
                    <%else%>
                        <span style="color:blue"><h3><% response.write sID %> is used by <%response.write sWho & "-" & sName %></h3></span>
                        <span style="color:red"><h4>Cannot delete this Grade</h4></span>
                    <%end if%>
                </div>
            </div>
        </div>
